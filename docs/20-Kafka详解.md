...
> 面向 .NET 工程师的 Kafka 深度学习，含 C# 实战 Demo。
...
| 库 | 说明 |
|----|------|
| **Confluent.Kafka** | 官方推荐，基于 librdkafka（C 库），性能最好 |
...
```
入门(2-3 天):
├─ 1. Docker Compose 启动 Kafka + ZooKeeper
├─ 2. kafka-topics 命令行操作 Topic
├─ 3. kafka-console-producer / consumer 收发消息
└─ 4. 跑本项目 examples/KafkaDemo

进阶(1-2 周):
├─ 1. C# 生产者 + 消费者完整集成
├─ 2. 分区 + 副本配置实验
├─ 3. Consumer Group 多消费者实验
├─ 4. 幂等生产者 + 事务实验
...
- 本项目代码：[examples/KafkaDemo](file:///c:/Users/ZJN/Desktop/jl/project/examples/KafkaDemo/Program.cs)
